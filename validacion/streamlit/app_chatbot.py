...
# ==========================================
#   3. HERRAMIENTAS (CON VISUALIZACIÓN DE DATOS)
# ==========================================
...
def api_search_movie(titulo):
    ...
    try:
        ...
        # --- DEBUG VISUAL ---
        with st.expander("🌐 Datos Crudos API (TMDb)", expanded=False):
            st.json(raw_data)
        # --------------------

        data = raw_data.get("results", [])[:1]
        if data:
...
def api_discover_movies(genre_id=None, year=None):
    ...
    try:
        ...
        # --- DEBUG VISUAL ---
        with st.expander("🌐 Datos Crudos Discover (TMDb)", expanded=False):
            st.write(f"URL: {url}")
            st.json(raw_data)
        # --------------------

        data = raw_data.get("results", [])[:5]
...
def elastic_text_search(concepto):
    ...
    try:
        ...
        # --- DEBUG VISUAL (Score de Texto) ---
        hits_debug = []
        for h in resp['hits']['hits']:
            hits_debug.append({
                "Score (BM25)": h['_score'],
                "Película": h['_source'].get('title'),
                "Sinopsis Parcial": h['_source'].get('overview', '')[:50] + "..."
            })
        
        with st.expander("🧠 Datos Elasticsearch (Texto/BM25)", expanded=False):
            st.write(f"Concepto buscado: '{concepto}'")
            st.dataframe(pd.DataFrame(hits_debug))
        # -------------------------------------
...
def search_script(frase):
    ...
    try:
        ...
        # --- DEBUG VISUAL (Vectores) ---
        hits_debug = []
        for h in resp['hits']['hits']:
            hits_debug.append({
                "Similitud (0-1)": h['_score'],
                "Película": h['_source']['title'],
                "Fragmento (EN)": h['_source']['chunk_text'][:100] + "..."
            })
            
        with st.expander("📐 Datos Vectoriales (RAG Guiones)", expanded=False):
            st.write(f"Frase vectorizada: '{frase}'")
            st.dataframe(pd.DataFrame(hits_debug))
            st.caption("Nota: Un score > 0.6 indica una coincidencia semántica muy fuerte.")
        # -------------------------------

        results = []
        for h in resp['hits']['hits']:
            src = h['_source']
            if h['_score'] > 0.4: # Filtro un poco más estricto
